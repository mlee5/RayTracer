...
struct pointLS *currLight = light_list;
  while (currLight != NULL)
  {
	...
	//fprintf(stderr,"direction %f %f %f %f\n", direction->px, direction->py, direction->pz, direction->pw);
    subVectors(p, direction);
	//fprintf(stderr,"direction %f %f %f %f\n", direction->px, direction->py, direction->pz, direction->pw);
   
    normalize(direction);
	//fprintf(stderr,"normalized %f %f %f %f\n", direction->px, direction->py, direction->pz, direction->pw);
    struct ray3D *shadowRay = newRay(p, direction);
	
	double intensityAmbient = 255;
    double intensityDiffuse = 255;
    double intensitySpecular = 255;

	lambda =  (double *) malloc (sizeof (double));
	pHit = newPoint(0, 0, 0);
	nHit = newPoint(0, 0, 0);
	//fprintf(stderr,"inside shade\n");
    findFirstHit(shadowRay, lambda, obj, &objHit, pHit, nHit, &a, &b);
	//fprintf(stderr,"back to shade shade\n");
	
	//fprintf(stderr," lambda in shade %f\n", *lambda);
 ...
 */
 ...
 // Be sure to update 'col' with the final colour computed here!
     fprintf(stderr, " colors that come out %f %f %f\n", col->R, col->G, col->B);
...
// while (curr_object != NULL && curr_object != Os)
	 while (curr_object != NULL && curr_object != Os)
 {
 ...
 */
  ...
  //temp to test, just pass in one from header later
  struct colourRGB *col2 = (struct colourRGB *) malloc (sizeof (struct colourRGB));

  
  rtShade(obj, p, n, ray, depth, a, b, col2);
  col->R = col2->R;
  col->G = col2->G;
  col->B = col2->B;
 ...
 for (j=0;j<sx;j++)   // For each of the pixels in the image
 {
  ...
  for (i=0;i<sx;i++)
  {
    ...
    *(rgbIm+(3*(sx*j+i)+2)) = (unsigned char)(col.B*255);
	  fprintf(stderr, " colors from main %f %f %f\n", col.R *255, col.G *255, col.B *255);
